...
{
	/* key_result's type is a result<string_view> which contains status and
	 * if status is equal to status::OK, contains also value (string_view in this
	 * case), for more info check result's documentation */
	auto key_result = it.key();
	/* check if status is ok, alternatively you can do:
...
{
	/* val_result's type is a result<string_view> which contains status and
	 * if status is equal to status::OK, contains also value (string_view in this
	 * case), for more info check result's documentation */
...
static void single_threaded_engine_example(const std::string &path)
{
	...
	/* Get a write range. Defaultly it is a whole value (pos = 0, n =
